...
- The technology stack, local interface form, summarization agent or model, data-sharing boundary, report time, retention period, and source-collection details are not yet approved. Do not select them silently. Bring choices that would settle these questions to Josh with concrete trade-offs.
...
- When a source cannot be read, show the report as incomplete. Do not interpret missing data as inactivity. Test each source and its failure behavior before describing it as supported.
