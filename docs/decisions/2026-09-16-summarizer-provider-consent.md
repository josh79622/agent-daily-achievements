...
Define what source records enter a day's summarization request, how complete inputs are handled if they exceed model limits, the model and output contract, and the report's status when no approved or working agent exists. Do not label a missing or failed model response as a complete daily report.
