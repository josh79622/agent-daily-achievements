...
**Update 2026-09-22 (read this first, supersedes the 2026-09-21 note below):** HEAD is `ca221dc`,
working tree clean, `npm run check` passes with 444 tests. A dev server may still be running on
port 4317 (`npm run dev`).

Done this session, each with an approved test-case doc under `docs/plans/` and its own commit:
L2 on-demand language packs (`10f79ff`), L3 right-to-left layout (`002e4d9`), L4 cached languages
stay added (`0495ee1`), Task 5 component tests (`1de8ee4`), S1 the 07:00 window and the launchd
schedule (`07a7726`), S2 the setup command (`2aaa151`). Full entries are above under each task.

State of Josh's machine, so the next session does not re-run anything by accident:
- `data/locales/` holds real packs for `ar`, `ja`, `zh-CN`. `ar` and `ja` were built by me with
  Josh's approval; `zh-CN` Josh built himself in the page. Do not delete them.
- `data/report-timezone.json` holds `Australia/Sydney`, written by `npm run setup`. Josh delegated
  the choice to me; the reasoning is in the S2 entry. Change it only with
  `npm run setup -- --force <zone>`.
- `data/reports/2026-09-20.json` was generated by the real scheduled run. The UI language and
  `data/summary-permission.json` were both returned to `zh-TW` after testing.
...
Two working-style points Josh set this session, both of which changed how I write: test-case docs
